...
triplet isBST(BinaryTreeNode<int> *root)
{
	...
	int MINy= min( root->data, min(leftOutput.MIN, rightOutput.MIN) );
	int MAXy= max( root->data, max(leftOutput.MAX, rightOutput.MAX) );
	
	bool checkBSTy= (root->data > leftOutput.MAX) && (root->data <= rightOutput.MIN) && leftOutput.checkBST && rightOutput.checkBST;
	
	triplet outputY;
	
	outputY.MIN= MINy;
	outputY.MAX= MAXy;
	outputY.checkBST= checkBSTy;
